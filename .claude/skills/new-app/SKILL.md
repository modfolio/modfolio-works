...
### 4. dotenvx 시크릿 세팅 (canon `secrets-dotenvx` v2.0+)

- `bun add -D @dotenvx/dotenvx@1.61.5`
- `.env` 작성 → `bunx --bun dotenvx encrypt -f .env --no-ops`
- `.env.keys` chmod 600 + `.gitignore` 에 추가 + 1Password 업로드 (`op document create .env.keys --title "dotenvx-{repo-name}-keys" --vault "modfolio-secrets"`)
- 동일 절차로 `.env.production` 생성
- `ecosystem.json.secretsMigration.completed` 에 등재 (`<repo>: { migratedAt, notes }`)
- ❌ Doppler 신규 도입 금지 (2026-04-25 폐기 확정)

### 5. 외부 서비스 등록

- Sentry: 새 프로젝트 -> DSN 을 해당 repo `.env` 에 (dotenvx encrypt)
- PostHog: 새 프로젝트 -> API 키를 해당 repo `.env` 에
- Resend: `mod@{domain}` 가입 -> API 키를 해당 repo `.env` 에
- Neon: DB 생성 -> 연결 문자열을 해당 repo `.env` 에
