...
- CSP 헤더 불완전
- 에러 정보 노출
- 동시성/멱등 (canon `concurrency-safety.md`): 결제·차감·상태전이의 TOCTOU(read-check-write) · partial write(tx 부재) · 멱등키 부재/랜덤/비원자 예약 · 웹훅 소비자 event dedupe 부재 — 중복 과금·재고 이중판매는 보안 사고와 동급으로 취급
- LLM/사용자 마크다운 렌더 (canon `llm-markdown-safety.md`): `marked`(및 대다수 md 파서) 기본값은 raw HTML **통과** — md 출력이 `{@html}`/`dangerouslySetInnerHTML` 에 닿는 경로 전수 확인. 요구 = 구성상 안전(raw HTML escape + 링크 http/https/mailto 허용목록 + 이미지→alt) + **렌더 모듈 단일화**(안전/비안전 이중문 = 사고). DOM 의존 새니타이저(isomorphic-dompurify 등)는 workerd 모듈 init throw → 인증 사용자만 SSR 500 — 금지. 테스트가 `not.toContain("onerror")` 류 약한 프록시면 오답(태그 전수 열거 + 메타 테스트여야)
