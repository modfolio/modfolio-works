...
**CLI 가 적재 시점에 검증한다 (v3.44.3+)** — 두 실패의 성격이 달라 판정도 다르다:
- **코퍼스에 없는 id → 거부(exit 1).** `curate.ts` 는 없는 id 를 조용히 건너뛰므로, 오타 하나로 그 세션의 신호가 통째로 사라지는데 아무도 모른다. 침묵하는 무효화라 통과시키지 않는다.
- **실존하지만 이 세션에 주입 안 된 id → 경고(통과).** `/playbooks` 로 직접 읽은 경우가 정당하기 때문이다. 다만 그 bullet 의 `helpful` 이 실제로 오르고 그게 자동 승격을 먹이므로, 경고가 뜨면 **정말 썼는지** 확인하라.
- **자동 채우기는 없다.** 주입에서 사용을 유추하지 않는다 — 그게 카운터를 부풀리는 바로 그 경로다.
