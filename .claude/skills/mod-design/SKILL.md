...
# /mod-design — Canva(클라우드) + forge(로컬 미디어) 디자인 파이프라인

> ## 왜 `mod-design` 인가 (2026-08-30 개명)
>
> 이름이 `design` 이었을 때 **우리 것인지 Claude 것인지 구분이 안 됐다.** 실측하니
> 공식 마켓플레이스의 25개 스킬과 우리 51개 사이에 **이름 충돌은 0건**이었고
> (Anthropic 것은 `frontend-design` 이다), 즉 가려진 적은 없었다 — 그런데
> **혼동은 실재했다.** 이름은 충돌이 아니라 **출처**를 말해야 한다.
>
> ⚠ 두 스킬은 **역할이 겹치지 않는다.** 이 문서가 아래에서 스스로 적는다 —
> *"이 스킬은 구현 파이프라인이지 디자인 가이드가 아니다."* 미학 방향·타이포·
> 「템플릿처럼 안 보이게」는 `frontend-design` 의 자리다. 둘을 같이 쓴다.
