...
# Opus Effort Policy — 권고 (baseline Opus 4.8)

> 이 문서는 **권고**이며 강제가 아니다. 각 앱은 자체 `.claude/settings.json`에서 override 가능.

> **버전 정합 (2026-06-10, v1.3.0)**: 이 정책은 **Opus 4.5+ 전 버전에 적용**되며, universe 현행 baseline 은 **Opus 4.8** (`claude-opus-4-8` / `claude-opus-4-8[1m]`, harness 3.5.0). 아래 모델 티어·agent 분포 표의 ID 는 현행 baseline(4.8) 기준이다. Opus 4.8 은 4.7 의 request surface 를 그대로 계승(adaptive thinking only, sampling/`budget_tokens` 제거 — claude-api skill: "no new breaking changes"). 본문의 "Opus 4.7" 산문 언급(토크나이저 +35%, adaptive thinking 출시 등)은 **4.7 출시 시점 배경**으로 보존. 파일명 `opus-4-7-effort-policy.md` 는 **안정적 cross-ref 식별자**로 유지한다 — 30+ append-only 이력 레코드 + 10여 active canon 링크가 이 이름을 참조하므로 rename 시 orphan 발생. (참고: 모델 출시 신기능 권고는 `claude-code-2026h1-features.md`.)

## 모델 티어

| 티어 | 모델 ID | Context | 용도 |
|------|---------|---------|------|
| Premium (1M) | `claude-opus-4-8[1m]` | 1,000,000 tokens | Figma metadata / 대형 diff / 대용량 컨텍스트 agent |
...
| # | Agent | 모델 | effort | 근거 |
|---|-------|------|--------|------|
| 1 | design-engineer | claude-opus-4-8[1m] | **max** | 디자인 의사결정 + Figma metadata 대용량 |
| 2 | security-hardener | claude-opus-4-8 | **max** | 보안 코드 (OWASP 취약점 — 오답 비용 = secret leak) |
| 3 | incident-handler | claude-opus-4-8 | **max** | P0 장애 triage + 포스트모템 |
| 4 | page-builder | claude-opus-4-8[1m] | xhigh | 레이아웃 (정형) — 2026-05-13 max → xhigh |
| 5 | component-builder | claude-opus-4-8 | xhigh | UI 컴포넌트 (정형) — 2026-05-13 max → xhigh |
| 6 | api-builder | claude-opus-4-8 | xhigh | 엔드포인트 + Zod (정형) — 2026-05-13 max → xhigh |
| 7 | schema-builder | claude-opus-4-8 | xhigh | Drizzle (정형) — 2026-05-13 max → xhigh |
| 8 | contract-builder | claude-opus-4-8 | xhigh | Zod contracts (정형) — 2026-05-13 max → xhigh |
| 9 | quality-fixer | claude-opus-4-8 | xhigh | 기계 수정 (정공법, 정형) — 2026-05-13 max → xhigh |
| 10 | code-reviewer | claude-opus-4-8[1m] | xhigh | 리뷰: 대규모 diff 1M |
| 11 | design-critic | claude-opus-4-8 | xhigh | 리뷰: Anti-Slop |
| 12 | architecture-sentinel | claude-opus-4-8 | xhigh | 리뷰: 불변 원칙 |
| 13 | accessibility-auditor | claude-opus-4-8 | xhigh | 리뷰: WCAG AA |
| 14 | migrations-auditor | claude-opus-4-8[1m] | xhigh | 리뷰: Drizzle 마이그 안전성 |
| 15 | test-builder | claude-opus-4-8 | high | 테스트 생성 |
| 16 | visual-qa | claude-opus-4-8 | high | Playwright + axe 5-gate |
| 17 | ecosystem-auditor | claude-opus-4-8 | high | ecosystem.json 검증 |
| 18 | perf-profiler | claude-opus-4-8 | high | CF Workers cost/latency |
...
| 22 | lead-planner (v2.35 P1.5 신규) | claude-opus-4-8 | xhigh | Multi-Agent Research Tier 1 — orchestration. trusted-input-only (lethal-trifecta 회피) |
| 23 | evaluator (v2.35 P1.5 신규) | claude-opus-4-8 | xhigh | Multi-Agent Research Tier 3 — 통합 verdict (binary pass/fail + weighted score). file modify 금지 |
| 24 | process-reward-evaluator (v3.0 P2.2 신규) | claude-opus-4-8 | high | PRM step-wise verifier — Generator step sequence 의 매 step 0-10 score. canon `process-reward-model.md` v1.0+ |
...
2. **1M context variant**: `claude-opus-4-8[1m]` 은 대형 codebase/diff 를 한 번에 담을 수 있지만, 그 큰 prefix 를 매 호출마다 재처리하면 비용이 선형 증가한다. `cache_control` breakpoint 를 frozen 부분 끝에 명시해야 효율이 나온다.
