...
> **파운더 지시 (2026-08-21 · 원문 비공개 — _quotes.md#CN-12)**: 앱·웹앱의 **기본 기능 목록**은 우리
> 코드로 직접 갖춘다. 모든 프로젝트는 최소 한 개, **그 목적에 맞는 고유 기술**을 갖춘다. 지금 쓸 곳만이
> 아니라 **모든 가능성에 대비해** 준비한다 — 나중에 조립해 앱을 찍어내기 위해서이고, 그것이
> **visualize the untold** 정신이다.
...
| 고객 상점 결제 운영(PortOne 상점 연결·웹훅 배선 검증·테스트 결제·정기결제 준비·건강 신호 · 부품 `@modfolio/merchant-ops` · 고객 PortOne 키는 Operator 런타임에서만) | modfolio-pay | claimed |
...
<!-- totals:begin -->
역량 69 · parts 48 · claimed 11 · open 10 · 주인 repo 28
<!-- totals:end -->
...
파운더가 명시한 조건: 역량을 억지로 떠안기는 배정이 아닐 것(원문 비공개 — _quotes.md#CN-33). 판별 질문 하나:
