...
> **분류는 per-segment** — 명령을 shell separator(`;`·`&&`·`|`·newline)·comment(`#`)로 쪼개 각 세그먼트 독립 분류. `echo`/`grep`/`cat` 등 inert-leading 세그먼트는 skip. → 디코이 우회(`stripe charges create # sk_test_`) 불가 + `echo 'terraform apply'` 오탐 없음. curl 은 순서 무관(curl+write-verb+host 독립 매칭).
