...
| `npm-registry` | nas | pkg.modfolio.io | forgejo-token | 1차 consume = GitHub Packages 단일 → **무영향** (pkg.modfolio.io 는 부차 채널) |
...
- ① `@modfolio/*` NAS 부차 npm registry(ADR-012, self-hosted 축) — 1차 consume 은 GitHub Packages 단일 유지.
- ② endpoint `pkg.modfolio.io`(401 = 인증요구 정상), 토큰 = `FORGEJO_NPM_TOKEN`. ⚠ `.npmrc` 는 scope 당 registry 1개(`project-infrastructure-registry.md` §dual-registry) — 소비 전환 설정 상세 = infra repo ADR-012 문의 (ecosystem 미문서화).
- ③ 다운 시: 1차 consume=GitHub Packages 단일 무영향(부차 채널).
