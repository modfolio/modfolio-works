...
- **wrangler 4.112~4.113 은 `@astrojs/cloudflare` 의 `legacy_env` 를 거부했다 — 4.114 에서 풀렸다(홀드 해제 · connect 2026-07-26).** 어댑터는 지금도 `legacy_env: true` 를 내보낸다(14.1.4 실측) — 관대해진 쪽은 wrangler 다. 그래서 «어댑터가 그만 내보내면 해제» 라는 옛 조건은 틀렸고, 어댑터 `dist/` 를 grep 하는 것도 근거가 아니다(빌드 시점 resolved config 에서 직렬화된다). **정직한 확인 = 실제 빌드가 만든 redirect config 에 `wrangler deploy --dry-run`**(connect 강제 uncached build 7/7). 2026-09-25 fleet 실물: `@astrojs/cloudflare` 14.3.x 앱들이 wrangler 4.131~4.136 에서 돈다(connect·atelier·pay). `~4.110.0` 홀드 핀은 더 두지 않는다.
