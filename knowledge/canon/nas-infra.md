...
| GitHub 미러 | **있음 — private DR 미러 (dual-push)** (2026-07-04 정정: 과거 "없음" 기술은 실제와 모순 — infra `origin` push 는 Forgejo+GitHub dual 이고 GitHub 은 DR fallback. secret-free·athsra 런타임 주입이라 토폴로지 민감성은 접근통제 미러로 수용 — infra 오너 결정, infra `docs/atlas-portal-plan.md §13.6`) |
...
| modfolio-infra | **dual-push** (fetch=Forgejo SoT, push=Forgejo+GitHub) | **DR fallback 수신 (dual-push)** — 2026-07-04 정정, 과거 "미러 없음" 모순 해소 | **primary (fetch SoT)** |
...
> **⚠ 2026-07-04 정정 2 — NAS-primary 실현 방식 = dual-push (infra 실측·오너 결정)**: 위 "서버 push-mirror" 비전은 2026-07-01 실측(26 repo 전부 `mirror:false`·stale = **silent-drift**)이 보여준 취약성 때문에 기본 경로에서 강등. 현행 표준 = **dual-push** — `origin` fetch=Forgejo(SoT)·push=dual(Forgejo+GitHub)·`github` fallback remote. dev push 시점에 양쪽이 항상-최신이라 drift 창 자체가 없다(과거 `c133764` divergence 는 셋업 미완이 원인). keepnbuild push-mirror 파일럿(journal `20260701-nas-primary-chain.md`)은 역사 기록으로 유지. 수신 = `feedback/modfolio-infra/2026-07-04_crossproject-findings.md` §1.
...
| `FORGEJO_NPM_TOKEN` | Forgejo npm registry **consume** (read:package). fleet 소비 토큰(2026-07-05 27 봉투 배포). |
| `FORGEJO_NPM_PUBLISH_TOKEN` | Forgejo npm registry **publish** (write:package). hub·connect 전용 — fleet 배포 금지(불변). |
...
athsra set modfolio-ecosystem FORGEJO_NPM_TOKEN=fjt_...          # read:package (consume, fleet)
athsra set modfolio-ecosystem FORGEJO_NPM_PUBLISH_TOKEN=fjt_...  # write:package (publish, hub·connect only)
athsra set modfolio-ecosystem ECOSYSTEM_SYNC_TOKEN=ghp_...
```

## npm registry — GitHub Packages (단일)

> ⚠ **§결정 2026-07-05 supersede**: **소비(consume) 기본 = `pkg.modfolio.io`(단일 registry)** 로 전환됨. 아래 sibling `.npmrc` 예시(GH Packages)는 이관 전 형태 = 비상 수동 스위치로만 유효. **publish 는 dual**(GH Packages canonical + pkg.modfolio.io mirror). 상세 = `registry-redundancy.md` §결정 · `pkg-registry-rollout.md`(infra SoT).
...
>
> **✅ 정정 2026-07-04 (infra crossproject-response + hub end-to-end 완결):** 위 2026-06-15 "**npm publish 로는 불가**" 는 **stale** — 뒤집혔다. 기전: pack → `publishConfig.registry` 를 pkg URL 로 **SET**(삭제 금지 — 삭제하면 상위 `.npmrc` 가 이겨 npmjs 로 감) → `npm publish` 가 `pkg.modfolio.io` 를 정확 타겟. 블로커였던 write 권한 = `forgejo admin user generate-access-token --scopes write:package` 로 해소(read 토큰 PUT 401 의 근본 = write:package 부재였음). least-privilege 분리: publisher `FORGEJO_NPM_PUBLISH_TOKEN`(write:package) · consumer `FORGEJO_NPM_TOKEN`(read:package). `harness-publish.ts [5/5]` 가 publishConfig override 로 dual-publish 실구현. **라이브 실측**: `@modfolio/harness` 3.17.2/3.17.3 → `pkg.modfolio.io`·`git.modfolio.io` 둘 다 GET 200(dist-tag latest=3.17.3). rollout doc `docs/pkg-registry-rollout.md` §2 정합. **✅ cutover 실행됨(§결정 2026-07-05)**: `@modfolio/*` 3종(harness 3.17.6·contracts 1.7.0·connect-sdk 8.7.0) pkg 라이브 → hub `sync-npmrc` STANDARD_NPMRC→pkg flip + read 토큰 27 fleet 봉투 배포(dev fleet-ready). (2026-07-04 'harness 1종·contracts 미게시' 는 stale — 이후 contracts 3.17.4·connect-sdk `97920b2` 게시.)
...
- `git push` (modfolio-infra): origin(Forgejo) 차단 시 **`git push github main` fallback 가능** (2026-07-04 dual-push 전환 — 과거 "fallback 없음" 기술 정정).
