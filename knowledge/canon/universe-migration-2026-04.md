---
title: Universe Migration 2026-04 — WSL native + harness v2.12.1 + 시크릿 표준 (athsra v3) 전환 가이드
version: 1.1.0
...
3. **시크릿 표준** (Doppler → dotenvx → **athsra v3**, 2026-05-02 universe-wide 최종 확정 — canon `secret-store.md` v1.13+. dotenvx 단계는 historical, Phase 3 archived)
...
| repo | WSL native | harness v2.12.1 | secrets (2026-04-25) | rename |
|---|---|---|---|---|
| **atelier-and-folio** | ✅ | ⏳ (v2.12.0, 재pull 대기) | ⏳ dotenvx migration in progress | ✅ (anf → atelier-and-folio 2026-04-25) |
| modfolio | ✅ | ⏳ (v2.12.0, 재pull 대기) | ❌ doppler | — |
| modfolio-connect | ✅ | ⏳ (v2.12.0, 재pull 대기) | ❌ doppler | — |
| modfolio-pay | ✅ | ⏳ (v2.12.0, 재pull 대기) | ❌ doppler | — |

> [2026-05-08 footnote] 표는 2026-04-25 snapshot. **현 표준 = athsra v3 (canon `secret-store.md` v1.13+, 2026-05-02)**. dotenvx 는 transitional, Phase 3 섹션 archived.
...
ls .env* 2>/dev/null                                   # 시크릿 흔적 (athsra v3 시대 평문 .env 없어야 정상)
bunx @athsra/cli ls <repo> 2>/dev/null | head -5       # athsra 등록 여부 (2026-05-02+ 표준)
...
**WSL GITHUB_TOKEN 빈 값 함정** (4 repo 전부 보고): WSL 이주 중 `~/.zshrc` 또는 Doppler sync 에서 `export GITHUB_TOKEN=…` 누락. 증상은 `echo $GITHUB_TOKEN` → 빈 문자열. `.npmrc` 의 `_authToken=${GITHUB_TOKEN}` 이 빈 값이면 `bun add -D @modfolio/harness` 가 401. 해결: 1Password / athsra / 기존 Doppler 에서 토큰 꺼내 `.zshrc` 또는 `athsra set <repo> GITHUB_TOKEN=…` 으로 추가.
...
# 2. 업그레이드 (athsra v3 표준, canon `secret-store.md` v1.13+)
athsra run <repo> -- bun add -D @modfolio/harness@2.12.1
# 미채택 repo: bun add -D @modfolio/harness@2.12.1 (athsra 채택 권고)
# 잔존 dotenvx (transitional): bunx --bun dotenvx run -f .env -- bun add -D @modfolio/harness@2.12.1
...
<details>
<summary>Phase 3 — Doppler → dotenvx 전환 [Archived 2026-05-08: athsra v3 표준 cement 후]</summary>

> 본 섹션은 2026-04-25 ~ 2026-05-02 의 universe-wide dotenvx 전환 절차 — **athsra v3 (canon `secret-store.md` v1.13+) cement 후 historical reference**. 신규 repo 는 athsra 직행. 잔존 dotenvx repo 의 athsra 마이그 절차는 `archive/secrets-dotenvx.md` § transition (또는 `~/code/athsra/scripts/migrate-package-json.ts`) 참조.
...
- Doppler `anf` → dotenvx (rename 필요 없음, 폐기됨) → 2026-05-02 athsra v3 로 최종 폐기 확정
...
# athsra v3 (canon `secret-store.md` v1.13+) — 2026-05-02 universe 표준
bunx @athsra/cli ls <repo> > /dev/null 2>&1 && echo "✅ athsra 등록" || echo "❌ athsra 미등록"
[ ! -f .env ] && echo "✅ 평문 .env 없음" || (head -1 .env | grep -q "DOTENV_PUBLIC_KEY=" && echo "⚠️ legacy dotenvx .env (athsra 마이그 권고)") || echo "❌ 평문 .env (시크릿 누출 위험)"
grep -qE "doppler run|dotenvx run" package.json && echo "❌ legacy run 명령 잔존" || echo "✅ legacy run 없음"

# 1Password (athsra master phrase backup)
op document get "athsra-master-phrase" > /dev/null 2>&1 && echo "✅ 1Password 업로드" || echo "❌ 1Password 업로드 필요"
...
- `knowledge/canon/secret-store.md` v1.13+ — athsra v3 시크릿 표준 (현행)
- `knowledge/canon/archive/secrets-dotenvx.md` v2.3.0 — dotenvx 상세 (archived 2026-05-02, historical reference)
...
3. `ecosystem.json.apps[]` 해당 엔트리에 `secrets: "athsra"` 추가 (legacy 값 `dotenvx`/`doppler` 은 historical)
