...
- **⚠ 복합 게이트의 제3축 부재 (2026-07-29 실측, Codex sol 리뷰 → 허브 확인)**: 위 두 축 어느 쪽도 **검색을 실행하지 않는다.** `planInjection` 호출 수 = `counter-gate.ts` **0** · `eval/run-pack.ts` **0** · `eval/ab-gate.ts` **0**. `buildArmBPrompt` 은 문항의 **정답 `pb_ids` 를 직접 주입**하므로(`run-pack.ts:43`) ab-gate 가 답하는 것은 *"올바른 카드가 주어지면 도움이 되나"* 이지 *"올바른 카드를 찾아내나"* 가 아니다. 오라벨은 아니다 — canon 이 이미 "코퍼스 수준"이라고 적었다. **문제는 합성**이다: 코퍼스가 좋고(ab-gate) 카운터가 성숙해도(counter-gate), 그 사이에 있는 **검색이 실사용에서 얼마나 맞히는지는 개통 판정에 들어가지 않는다.** counter-gate 의 `injectedCards`(`counter-gate.ts:101`)는 *레일이 도는가*만 보고 *맞게 찾는가*는 안 본다. 실사용 실측(2026-07-29): 발화 **1003/2533 = 39.6%**, 표본 6건 정밀도 **정확 1 · 관련 3 · 오탐 2**. → 제3축 = **실사용 라벨 평가셋**(`memory/evalsets/muse-live-prompts.json` + `scripts/muse/label-live.ts`). 라벨이 서면 `PASS = counter-gate AND ab-gate AND 검색 축` 으로 확장한다. 그때까지 fleet 개통은 **두 축만으로 판정하지 않는다**(=HOLD 유지).
