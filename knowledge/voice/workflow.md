...
- **observed**: 2026-09-21
- **quote**: «claude 쪽에서 알고 있는건 우리도 다 알아야 하니까 제대로 잘 공부해주고 분석하고 우리도 다 알아야해. 사실상 modfolio.md를 통해서 이런 것들도 다 제대로 기록이 남고 판단이 제대로 되어야해. 그리고 무엇보다도 내가 이렇다 저렇다 하는 판단들이 최신으로 항상 갱신이 되어야해.»
- **scope**: fleet
...
- **observed**: 2026-09-21
- **quote**: «기존 소유권 안에서 필요한 호환 가능한 B 확장은 자동으로 진행하도록 캠페인 권한을 정의합니다.» · «지금 pdgd가 개발 중이긴 하지만 그래도 신 modfolio 개발 환경 세팅이 끝나면 바로 모든 프로젝트에서 사용 가능하도록 전역에 다 최신으로 업데이트 해주고 커밋푸쉬해주고 제대로 작동하도록 해줘야해.»
- **scope**: fleet
...
- **observed**: 2026-09-21
- **quote**: «gemini가 읽고 그거를 분석한 결과만 codex에게 전달해야하는거 아닌가?» ·
  «제대로 세팅해서 확실하게 modfolio.md 세팅을 해주고»
- **scope**: fleet
...
- **observed**: 2026-09-21
- **quote**: «멀티 ai 프로그램을 연결하고 조합하는 상황이라면 사용량이 부족했을 때 fallback model로 돌려서 사용하는 방식도 구축은 해둬야 겠네.»
- **scope**: fleet
...
- **observed**: 2026-09-22
- **quote**: «claude가 메인이 되는 구조가 아닌건 알지? 앞으로 발전해나갈 modfolio.md를 주축으로 해서 어떤 도구에서든지 (claude desktop, antigravity, chatgpt desktop) 세팅된 각 역할별로의 ai가 작업을 해서 종합적으로 프로젝트를 개발 할 수 있게끔 해줘야해. 물론 실행된 도구에서 구동되는 ai는 그 모든것을 그때그때 orchestrate해야하는 의무는 있겠지만. 여기서의 소모되는 사용량은 뭐 어쩔 수 없지 않을까 싶긴 한데, 단순 orchestrate니까 어쨌든 ai를 낮은 effort로 해도 되겠지?» · «이상적인 modfolio.md 를 주축으로 하는 하네스와 이 환경은 어쨌든 내가 뭘 복잡하게 고민하지 않아도 그냥 예를들어 claude desktop을 켜서 세션을 시작하고 그냥 "이거 개발해줘" 라고만 해도 알아서 하네스 대로 개발이 이루어 져야하는거야. 그거에 대한 세팅을 원하는거고 지금.»
- **scope**: fleet
- **applies**: 정본은 `MODFOLIO.md` 하나이고 Claude Desktop/Code · Antigravity · ChatGPT Desktop/Codex 는 동등한 진입 표면이다 — 어느 공급자도 «메인» 이 아니다. 오너가 연 도구의 대화 모델이 그 순간의 orchestrator 이며, 역할 워커는 `config/ai-routing.json` 프리셋으로 배정한다. orchestration 은 기본 low effort 로 돌리고 그 사용량은 감수한다. 다만 두 지점은 단순 orchestration 이 아니므로 low 로 판정하지 않는다: ⓐ 기능 소유자 판정·소유자 간 A/B 분해(Atlas 가 모호할 때) ⓑ 리뷰 findings 가 0 이 아닐 때의 처분 — `triage` 프리셋으로 넘기거나 그 턴만 effort 를 올린다. 통합 승인은 계약·게이트가 판정하므로 orchestrator effort 와 무관하다. 인수 기준은 오너 문장 그대로다 — 어떤 도구를 켜서 «이거 개발해줘» 라고만 해도 시작 주문·모델 플래그 없이 하네스 흐름(소유자 판정 → 역할 배정 → 격리 구현 → 독립 리뷰 → 검증 → 통합 → 회신)이 이어져야 한다; 문서 배포만으로 이것을 완료로 보고하지 않는다. 「ChatGPT Desktop 을 기본 control plane 으로」(2026-09-21 구현 문서 `20260921-model-routing.md`)는 이 판단으로 정정한다. 같은 날 오너가 pdgd 도 이번 등록 물결에 포함하라고 했으므로 2026-09-21 의 «PDGD 제외» 는 rollout 범위에서 해제된다(pdgd 세션의 자율은 그대로).
- **status**: active
...
- **observed**: 2026-09-22
- **quote**: «지금 이 설계도 영 뭔가 좀 애매한거 같은데. 어차피 사용량은 내가 보면서 가이드 해줄건데... 우리가 뭐 사용량 까지 막 보면서 개발을 해야하나? 다 떨어지면 어쩔 수 없는거지... 30% 한도였나? 그런거도 없애고 어차피 토큰 다 사라질때까지 계속 작업은 돌리는거야. 물론 1주일 내내 개발이 멈추면 안되지.»
- **scope**: fleet
- **applies**: 사용량은 오너가 보면서 가이드한다. 하네스는 공급자 사용량을 최신으로 **보고**(statusline·usage API)하되, 예약 용량(30%)·미측정·오래된 스냅샷·창 부족을 이유로 작업을 막거나 모델을 낮추지 않는다(`config/ai-routing.json` `usagePolicy.gate: advisory` — 판정은 경고로 남는다). 유지되는 것: 유료 API 전환·reset credit·업그레이드 금지, 동시 실행 상한(박스 보호), 고위험 역할의 품질 바닥, 독립 실행 리뷰, 사용량이 실제로 소진돼 공급자가 거절하면 역할에 맞는 대체 모델로 이어가는 것. «1주일 내내 개발이 멈추면 안 된다» 는 하네스의 차단이 아니라 오너의 페이스 판단이며, 주간 창 소진 경고를 보고에 남긴다. 이전 두 판단(Codex 주간 예약 · Claude 0%/누락 구분)은 이 판단으로 승계된다.
- **status**: active
